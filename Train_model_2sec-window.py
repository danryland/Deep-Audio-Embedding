...
x = Conv1D(32, 3, strides=1, activation='relu', padding='valid', kernel_initializer='RandomNormal', bias_initializer='zeros')(encoder_input)
...
x = Dense(352, activation='relu', use_bias=False, kernel_initializer='RandomNormal')(decoder_input)
x = Reshape((11, 32))(x)
x = UpSampling1D(2, name='decoder_upsample1')(x)
x = Conv1D(64, 3, strides=1, activation='relu', padding='same', kernel_initializer='RandomNormal', bias_initializer='zeros')(x)
x = BatchNormalization(axis=-1)(x)
x = UpSampling1D(2, name='decoder_upsample2')(x)
x = Conv1D(128, 3, strides=1, activation='relu', padding='same', kernel_initializer='RandomNormal', bias_initializer='zeros')(x)
x = BatchNormalization(axis=-1)(x)
x = UpSampling1D(2, name='decoder_upsample3')(x)
z_decoded = Conv1D(img_height, 1, strides=1, activation='sigmoid', padding='same', kernel_initializer='RandomNormal', bias_initializer='zeros')(x)
...
vhat = Dense(latent_dim, activation='tanh', name='vhat')(w)  # Predicted target

# Input branch 2: compute target
input_top_frames = Input(shape=top_frames_shape, name='input_top_frames')  # Input is (batch, 2 tensor slices, width:timesteps, height:frequency)
...
x_i = input_top_frames[:, 0, :, :]  # take frame at current time-step # Input is (batch, 2 tensor slices, width:timesteps, height:frequency)
x_i_hat = Decoder(z_i)
