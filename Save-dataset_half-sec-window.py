...
window_size = 1024  # samples. freq resolution increases and time resolution decreases over increasing window size.
...
secs_per_img = 0.5  # NOTE: Changing this will change the image dimensions, which will cause the decoder network to throw error. length of time (s) to ultimately reduce to a single point in latent space

window = signal.windows.tukey(window_size, alpha=0.4)
F, T, S_mag = signal.spectrogram(X, sample_rate, scaling="spectrum", mode="magnitude", window=window,
                                 noverlap=N_overlap)
...
# dt = int(np.round(dt_secs*time_resolution))  # number of spectro time bins between tensor time slices, and also separating consecutive embedded points
...
f_max = 4000  # 2100  # max frequency (Hz) to include in spectrogram. dont change
...
np.savez(outfile, F=F_crop, T=T, S_mag=S_mag_crop, dt=dt, img_width=img_width, img_height=img_height,
...
plt.pcolormesh(T[:img_width * c], F_crop[:img_height], S_mag_crop[:img_height, :img_width * c], shading='nearest',
               cmap='inferno')
